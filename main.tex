...
restritivo e submetidos a revisão manual, que verificou: (i) a correção
gramatical das sentenças; (ii) a validade da relação hiponímica entre os
termos substituídos; (iii) a ausência de outros mecanismos inferenciais além
da inclusão lexical (e.g., negação, pressuposição ou implicatura); e (iv) a
adequação dos casos de não-acarretamento, assegurando que a hipótese não
decorra da premissa por nenhum outro mecanismo semântico formal. O modelo
...
vieses de familiaridade. Por construção, o dataset sintético isola o
comportamento dos modelos em inferências baseadas em inclusão lexical, sem
regularidades superficiais do \emph{benchmark}. Dado seu tamanho reduzido e
escopo temático único (60 pares, domínio do futebol, fenômeno restrito à
hiponímia/hiperonímia), o conjunto constitui uma prova de conceito
metodológica para avaliação controlada --- e não uma avaliação abrangente do
acarretamento semântico em sua totalidade.
...
A Tabela~\ref{tab:sintetico} apresenta os resultados no dataset sintético,
construído exclusivamente com pares baseados em hiponímia.
...
\textbf{Padrões comportamentais e a distinção dedução/indução.}\ A tipologia
de Peirce oferece uma moldura interpretativa para os comportamentos observados.
Modelos que colapsam para uma classe --- Sabiá (sempre ``não acarreta''), Bode
(sempre ``acarreta''), Ministral-8B-Instruct (sempre ``não acarreta'') ---
apresentam padrões de resposta incompatíveis com a verificação das condições
de verdade que define o acarretamento: a decisão permanece constante
independentemente do conteúdo proposicional do par. Esse comportamento é
observacionalmente consistente com o de classificadores baseados em viés de
classe, nos quais a distribuição léxico-semântica atua como \emph{proxy} para
a decisão inferencial em lugar de uma avaliação das relações lógicas entre
premissa e hipótese. É importante ressaltar, contudo, que os experimentos
medem exclusivamente comportamento observável em tarefas de classificação;
inferir a partir dessas métricas a presença ou ausência de capacidades de
raciocínio interno extrapola o que os dados permitem concluir diretamente.
Esse comportamento reproduz as mesmas limitações das heurísticas simbólicas
identificadas por~\cite{stil}: a distinção entre similaridade distribucional e
preservação de condições de verdade permanece opaca para modelos sem
mecanismos explícitos de raciocínio.

\textbf{Raciocínio explícito e padrões de resposta mais robustos.}\ Os melhores
modelos --- Qwen3, Gemma~2 e variantes \emph{Reasoning} do Ministral ---
exibem padrões de resposta mais consistentes com os requisitos da tarefa:
mantêm alto desempenho tanto nas faixas de baixa similaridade (rejeição
correta de pares dissimilares) quanto na faixa~5 (aceitação correta de
paráfrases), e preservam essa robustez nos dados sintéticos. Esses resultados
sugerem que a \emph{Chain-of-Thought} e a destilação de conhecimento produzem
representações comportamentais mais alinhadas à distinção entre acarretamento
e similaridade distribucional --- embora não seja possível, a partir dos dados
de classificação disponíveis, determinar se esse alinhamento resulta de
raciocínio dedutivo genuíno ou de padrões aprendidos durante o treinamento que
mimetizam esse comportamento.
...
controlados. A superioridade do modo \emph{instruct} no ASSIN~2 pode sugerir
exploração mais eficiente de padrões superficiais do \emph{benchmark}; contudo,
dada a proximidade entre os valores (2,2 p.p.), essa diferença não deve ser
interpretada sem análise de significância estatística.

\textbf{A anomalia do Ministral-8B-Instruct.}\ A disparidade entre
Ministral-3B-Instruct (88,0\%) e Ministral-8B-Instruct (44,5\%) é
contra-intuitiva perante as leis de escala. Com base nas informações do
relatório técnico do Ministral~3~\cite{liu2026ministral3}, uma hipótese
explicativa é que o processo de alinhamento ODPO no modelo de 8B, ao otimizar
concisão e segurança para assistentes de \emph{chat}, possa ter suprimido
padrões inferenciais mais elaborados --- enquanto a destilação de
\emph{logits} específica aplicada ao modelo de 3B, combinada com a arquitetura
de \emph{tied embeddings}, teria mitigado esse efeito. É imprescindível, no
entanto, ressaltar que essa interpretação é especulativa: não há experimentos
de ablação ou avaliações controladas neste trabalho destinados a verificá-la
empiricamente. Estudos futuros com variações sistemáticas dos parâmetros de
alinhamento seriam necessários para confirmar ou refutar a hipótese.

\textbf{Análise qualitativa de erros.}\ A Tabela~\ref{tab:erros} ilustra como
o colapso de classe se manifesta concretamente: Sabiá-7B nega o acarretamento
mesmo quando a relação hiponímica é transparente (\emph{lateral-esquerdo}
$\subset$ \emph{jogador}); Bode-7B afirma o acarretamento mesmo quando os
predicados são de classes incompatíveis (\emph{meia} $\not\to$ \emph{árbitro}).
O erro não decorre de dificuldades com o vocabulário, mas de viés decisório
independente do conteúdo semântico do par.

\begin{table}[htbp]
\centering
\caption{Predições corretas (\checkmark) e incorretas (\texttimes) no dataset
sintético.}
\label{tab:erros}
\small
\begin{tabular}{p{3.5cm}p{3.5cm}ccc}
\toprule
\textbf{Premissa} & \textbf{Hipótese} & \textbf{Rótulo} &
\textbf{Sabiá} & \textbf{Bode} \\
\midrule
O lateral-esq.\ sofreu a falta. &
O jogador sofreu a falta. &
acarret. & \texttimes & \checkmark \\
O meia organizou o meio-campo. &
O árbitro organizou o meio-campo. &
não-acarret. & \checkmark & \texttimes \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{zero-shot} oferecem, portanto, uma alternativa viável ao ciclo custoso de anotação e treinamento supervisionado.
...
As principais limitações são: (i) o dataset sintético (60 pares, hiponímia)
constitui uma prova de conceito de escopo restrito, não generalizável ao
acarretamento em sua totalidade; (ii) os experimentos avaliam comportamento
observável em classificação binária --- inferir a presença de raciocínio
dedutivo interno a partir dessas métricas extrapola o que os dados permitem
concluir diretamente; (iii) comparações entre modelos de desempenho próximo
(e.g., 90,8\% \emph{vs.} 89,0\%) carecem de testes de significância
estatística; e (iv) as interpretações causais sobre modelos específicos
(notadamente o Ministral-8B-Instruct) são hipóteses baseadas em relatórios
técnicos, não verificadas empiricamente neste estudo.

Como trabalhos futuros, vislumbra-se: a ampliação do dataset sintético para
outros fenômenos da semântica formal (negação, quantificação, pressuposição,
predicação, operadores modais); a inclusão de testes de significância
estatística para comparações entre modelos; a realização de estudos de ablação
para investigar as causas do colapso em modelos específicos; e a avaliação em
regime \emph{few-shot} para quantificar o ganho decorrente de exemplos
explícitos.
